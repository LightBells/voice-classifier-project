...
subject_dirs = list(
    filter(lambda x: re.search(r"Speaker\d{4}", x) is not None, dirs)
)


def replace_dirname(x: str):
    return re.sub(r"Speaker(\d{4})", "Speaker_\1", x)
...
files = glob(
    os.path.join(args.target_dir, "50_speakers_audio_data/**/**.wav"),
    recursive=True,
)
subject_files = list(
    filter(lambda x: re.search(r"Speaker\d+_\d+.wav", x), files)
)


def replace_filename(x: str):
    return re.sub(r"Speaker(\d+_\d+).wav", "Speaker_\\1.wav", x)
